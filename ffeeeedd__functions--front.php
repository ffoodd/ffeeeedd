...
/*
  ...
  == Remplace le code généré par [caption]
*/
  ...
  */
   ...
   * @see : http://stackoverflow.com/questions/12279113/recommended-wai-aria-implementation-for-navigation-bar-menu
  ...
  class ffeeeedd__walker extends Walker_Nav_Menu {
    function start_el( &$output, $item, $depth = 1, $args = array(), $id = 0 ) {
      ...
      $output .= $indent . '<li' . $id . $value . $class_names .' role="menuitem" aria-labelledby="item-' . esc_attr( apply_filters( 'the_title', $item->title, $item->ID ) ) . '">';
      ...
      $item_output .= '<a'. $attributes .' id="item-' . esc_attr( apply_filters( 'the_title', $item->title, $item->ID ) ) . '">';
